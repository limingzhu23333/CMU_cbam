    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(loader):
            ...
            if save_attention and batch_idx == 0:
                torch.save(ca, f'logs4/{exp_name}/channel_attn_epoch_{epoch}.pt')
                torch.save(sa, f'logs4/{exp_name}/spatial_attn_epoch_{epoch}.pt')
...
if __name__ == '__main__':
    ...
    os.makedirs(f'logs4/{exp_name}', exist_ok=True)
    os.makedirs(f'logs4/{exp_name}/model', exist_ok=True)
    writer = SummaryWriter(f'logs4/{exp_name}/log')
    save_path = f'logs4/{exp_name}/model/best.pkl'
    latest_path = f'logs4/{exp_name}/model/latest.pkl'
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        dice, iou, sensitivity, accuracy, f1, loss = evaluation(epoch, val_loader, model, writer)
        val_loss_writer.writerow([epoch, loss])
        val_loss_file.flush()

        # Write new metrics to the CSV file
        metrics_writer.writerow([epoch, dice, iou, sensitivity, accuracy, f1])
        metrics_file.flush()
